...
def test_per_category_weights():
    ...
    # scalability:               BA=8,  AI SE=7, BA_w=70%, AI_SE_w=30% -> 8*0.7 + 7*0.3 = 5.6 + 2.1 = 7.7
    # deployment_readiness:      BA=4,  AI SE=3, BA_w=70%, AI_SE_w=30% -> 4*0.7 + 3*0.3 = 2.8 + 0.9 = 3.7
    # Total raw = 7.7 + 8.7 + 7.3 + 3.3 + 7.7 + 8.7 + 7.7 + 3.7 = 54.8
    # Final = (54.8 / 80) * 100 = 68.5
    ...
    assert result["head_judge_verdict"] == "HONORABLE_MENTION", \
        f"Expected 'HONORABLE_MENTION', got '{result['head_judge_verdict']}'"
    ...
    assert cats["scalability"]["dominant_judge"] == "BA"
    assert cats["deployment_readiness"]["dominant_judge"] == "BA"
